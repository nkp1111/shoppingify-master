...
const AddItemForm = () => {
  ...
  const { setShowAddItemForm, state: { foodItems }, addNewItem } = useGlobalContext()

  const handleForm = () => {
    setShowAddItemForm(false)
  }

  const getCategories = (foodItems) => {
    let categories = foodItems.map(item => item.category)
    categories = categories.filter((item, ind) => categories.indexOf(item) === ind)
    return categories
  }
